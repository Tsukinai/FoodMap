...
export async function GET(request: Request) {
  ...
  const filter = searchParams.get('filter') // 'visited' | 'want' | null = all
  ...
  // Global counts always reflect the full collection
  const total = rows.length
  const visited = rows.filter(r => r.status === 'visited').length
  const want = rows.filter(r => r.status === 'want').length

  // All breakdowns respect the filter
  const subset = filter === 'visited' ? rows.filter(r => r.status === 'visited')
    : filter === 'want' ? rows.filter(r => r.status === 'want')
    : rows

  const withCostMin = subset.filter(r => r.cost_min != null)
  const withCostMax = subset.filter(r => r.cost_max != null)
  ...
  const recent = subset.slice(0, 5).map(r => ({
    id: r.id, name: r.name, status: r.status, rating: r.rating, created_at: r.created_at,
  }))

  return NextResponse.json({ total, visited, want, avgCostMin, avgCostMax, byRating, byCuisine, byArea, byTaste, byScene, recent } satisfies DashboardStats)
}
